...
class retur extends MY_Controller {
    ...
    public function daftar_retur_pembelian()
    {
        ...
        $data['halaman'] = $this->load->view('retur/menu', $data, TRUE);
...
public function cari_nota()
{
  ...
  $this->db->select('*') ;
  $query = $this->db->get_where('transaksi_pembelian',array('kode_pembelian'=>$nomor_nota));
  $data = $query->row();
...
$list_supplier = $pembelian->row();
}
echo $hasil.'|'.$nota.'|'.$kode.'|'. $list_supplier->kode_supplier;
}
...
public function ubah_item_retur_temp()
{
 ...
 $this->db->select('*') ;
 $cek_transaksi = $this->db->get_where('transaksi_pembelian',array('kode_pembelian'=>$masukan['nomor_nota']));
 $hasil_cek_transaksi = $cek_transaksi->row();
 $kode_pembelian = $hasil_cek_transaksi->kode_pembelian;

 $this->db->select('*') ;
 $cek_opsi_transaksi = $this->db->get_where('opsi_transaksi_pembelian',array('kode_pembelian'=>$kode_pembelian,'kode_bahan'=>$masukan['kode_bahan']));
...
public function simpan_retur()
{
    ...
    foreach ($query_retur_temp->result() as $item){
        ...
        $cek_opsi_transaksi = $this->db->get_where('opsi_transaksi_pembelian',array('kode_pembelian'=>$input['nomor_nota'],'kode_bahan'=>$item->kode_bahan));
        $hasil_cek_opsi_transaksi = $cek_opsi_transaksi->row();

        $opsi_pembelian['jumlah']=@$hasil_cek_opsi_transaksi->jumlah - $item->jumlah;
        $opsi_pembelian['jumlah_retur']=@$hasil_cek_opsi_transaksi->jumlah_retur + $item->jumlah;
        $opsi_pembelian['subtotal']=@$hasil_cek_opsi_transaksi->subtotal - $item->subtotal;
        $opsi_pembelian['subtotal_retur']=@$hasil_cek_opsi_transaksi->subtotal_retur + $item->subtotal;

        $this->db->update('opsi_transaksi_pembelian',$opsi_pembelian,array('kode_pembelian'=>$input['nomor_nota'],'kode_bahan'=>$item->kode_bahan));
        //echo $this->db->last_query();
    ...
    if(@$transaksi_stok){
        ...
        $cek_pembayaran=$this->db->get_where('transaksi_pembelian',array('kode_pembelian'=>$input['nomor_nota']));
        $hasil_cek=$cek_pembayaran->row();
        if ($hasil_cek->proses_pembayaran=='credit') {
            $cek_hutang=$this->db->get_where('transaksi_hutang',array('kode_transaksi'=>$input['nomor_nota']));
            $hasil_hutang=$cek_hutang->row();

            $update_hutang['nominal_hutang']= $hasil_hutang->nominal_hutang -  $grand_total ;
            $update_hutang['sisa']= $hasil_hutang->sisa -  $grand_total ;
            $this->db->update('transaksi_hutang',$update_hutang,array('kode_transaksi'=>$input['nomor_nota']));
                //echo $this->db->last_query();
